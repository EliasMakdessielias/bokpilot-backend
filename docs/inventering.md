...
Totalsiffror (public-schemat, räknade ur databasen 2026-09-09): **127 tabeller** (samtliga med RLS aktivt), **0 vyer**, **262 egna funktioner** (exkl. extension-ägda), **85 triggrar**, **166 RLS-policies**, **198 index** (som inte backar constraints), **92 migrationer**, **32 edge functions**, **9 pg_cron-jobb**, **6 storage-buckets**. (Uppdaterat 2026-09-09 efter etapp 11b/11c — driftvakten bedömer bara avslutade körningar och `imap-import` är avregistrerad — samt kundappens tre säkerhetsmigrationer samma dag: `byrastod_scope_v1`, `ai_kvot_v1`, `inbound_avsandare_v1` (se sista avsnittet); edge-funktionerna synkade mot driftsättningen 2026-09-07. Dessförinnan 2026-09-02 efter etapp 10–16: backupunderlag, driftövervakning, rollstyrning på lönetabellerna, KYC-datamodell med bucket, KYC-bevakning, append-only-skydd av operatörsloggen samt KYC-arkiv vid avveckling. Dessförinnan 2026-08-25 efter etapp 4–9: search_path-låsning och anon-indragning, täckande FK-index, RLS-InitPlan-omskrivning, behörighetskoll utan uid-beroende, indragna tabellrättigheter för anon, BFL-spärr vid bolagsradering samt nattlig avstämning mellan databas och Storage.) Inga egna enum-typer. Enda schema med egna objekt är `public`.
...
**Registret.** `driftkomponenter` (namn som primärnyckel, `typ`, `max_tyst_timmar`, `max_fel_i_rad`, `aktiv`, `senast_rapporterad_status`) skiljer på två sorters komponenter, eftersom de kräver två sorters larm: för `cron` är tystnad felet (jobbet ska köra på schema), för `handelsestyrd` kan tystnad vara normalt (inga fakturor kom in) medan upprepade fel alltid är fel. Tabellen har RLS utan policies och alla rättigheter återkallade för `anon`/`authenticated` — bara `service_role`/`postgres` når den. Innehållet är data (2026-09-09: 15 komponenter, varav 13 aktiva; `folio-ocr` är avstängd i registret sedan 2026-08-26 eftersom ingen OCR-tjänst är driftsatt, och `imap-import` sedan etapp 11c eftersom IMAP-importen togs bort 2026-09-07) och speglas inte i schemafilerna.
...
## Kundappens säkerhetsåtgärder 2026-09-09 – byrastod_scope_v1, ai_kvot_v1, inbound_avsandare_v1

Tre migrationer från arbetet i kundappens repo (`EliasMakdessielias/bokpilot`; go-live-listans punkt 7 — de öppna fynden från edge-funktionsgranskningen 2026-08-25). Spegelfilerna finns även i kundappens `supabase/*.sql` med identiskt innehåll; edge-koden som använder dem deployas av GitHub Actions vid nästa push.
