...
Observation ur databasen: migrationen återkallar rättigheterna på de nya tabellerna från `public` och `anon` men inte från `authenticated`, och Supabases standardrättigheter hade redan gett den rollen full DML när tabellerna skapades. De uttryckliga `grant select, insert(, update)` är additiva, så `authenticated` har i praktiken samtliga tabellrättigheter på `kyc_huvudman` och `kyc_bilagor` (så redovisas de i `schema/grants.sql`); det är RLS-policyerna, utan delete, som hindrar radering. `kyc_arkiv` (etapp 16) gjordes rätt: `revoke all` inklusive `authenticated` följt av `grant select`.
