...
Totalsiffror (public-schemat, räknade ur databasen 2026-09-09): **128 tabeller** (samtliga med RLS aktivt), **0 vyer**, **264 egna funktioner** (exkl. extension-ägda), **85 triggrar**, **166 RLS-policies**, **199 index** (som inte backar constraints), **94 migrationer**, **33 edge functions** (varav `backup-underlag` väntar på deploy), **10 pg_cron-jobb**, **6 storage-buckets**. (Uppdaterat 2026-09-09 efter etapp 11b/11c — driftvakten bedömer bara avslutade körningar och `imap-import` är avregistrerad — samt kundappens fem migrationer samma dag: `byrastod_scope_v1`, `ai_kvot_v1`, `inbound_avsandare_v1`, `ai_kvot_v2` och `backup_underlag_v1` (se de två sista avsnitten); edge-funktionerna synkade mot driftsättningen 2026-09-07. Dessförinnan 2026-09-02 efter etapp 10–16: backupunderlag, driftövervakning, rollstyrning på lönetabellerna, KYC-datamodell med bucket, KYC-bevakning, append-only-skydd av operatörsloggen samt KYC-arkiv vid avveckling. Dessförinnan 2026-08-25 efter etapp 4–9: search_path-låsning och anon-indragning, täckande FK-index, RLS-InitPlan-omskrivning, behörighetskoll utan uid-beroende, indragna tabellrättigheter för anon, BFL-spärr vid bolagsradering samt nattlig avstämning mellan databas och Storage.) Inga egna enum-typer. Enda schema med egna objekt är `public`.
...
Etapp 9 konstaterade att Supabases databasbackuper inte omfattar Storage: filerna i bucketarna `underlag`, `arkiv`, `annual-report-exports` och `support` — räkenskapsinformation enligt BFL 7 kap. — hade ingen kopia alls (27 objekt 2026-09-09). Etapp 10 gav backupunderlaget `lista_lagringsobjekt()`; beslutsunderlaget `docs/arkivlagring-azure-verifiering.md` (2026-08-26) verifierade Azure Blob Storage i Sweden Central (Cold + ZRS + oföränderlig container) som mål. Kundappens edge-funktion `backup-underlag` (byggd 2026-09-09, deployas av GitHub Actions vid nästa push) kopierar nya och ändrade objekt dit varje natt; data lämnar aldrig Sverige (edge i Stockholm → Sweden Central, ingen mellanhand). Aktivering kräver ett Azure-konto i BokPilot AB:s namn och edge-secreten `AZURE_BLOB_SAS_URL` (container-SAS med skapa/skriv men aldrig radera; funktionen vägrar en SAS med `d` eller som gått ut) — se `docs/BACKUP-UNDERLAG.md` i kundappen.
