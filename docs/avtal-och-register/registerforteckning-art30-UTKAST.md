...
| Microsoft 365 (e-post, Teams, OneDrive/SharePoint); säkerhetskopia av Storage i Azure Blob Storage (i drift sedan 2026-09-09) | Microsoft Ireland Operations Ltd | EU Data Boundary; Azure-regionen Sweden Central (Sverige) för arkivet | A2, A4, A5, A7, B1, B6, B8 |
...
| `konsol_audit_logg`, `mcp_audit_log`, `ai_usage_log`, `stripe_event_log`, `notification_queue`, `notification_deliveries`, `notification_provider_logs`, `notification_events` | Operatörsåtgärder, MCP-anrop med parametrar, AI-användning per bolag (plangränser), betalhändelser, notisutskick (e-post, telefon) | [KLAMMER — fastställ; ingår i dag inte i gallringsjobbet. Förslag: 24 månader, `stripe_event_log` 7 år om händelserna utgör bokföringsunderlag] |
| `ai_call_log` | Räknare för AI-kvoter (utökad 2026-09-09): användar-id, bolags-id (endast support-ai), anropande funktion, tidpunkt — inget innehåll. Ändamål: skydd mot missbruk och kostnadstak (tolkning 8 anrop/användare/minut; AI-assistenter 60 anrop/användare/timme, 400/dygn, 1 500/timme för plattformen). Rättslig grund: berättigat intresse (art. 6.1 f). Läses enbart av `service_role`. | 12 månader (beslut 2026-09-09; gallras automatiskt av det nattliga gallringsjobbet sedan migrationen `gdpr_gallring_ai_call_log_v1`; intresseavvägningen dokumenteras i avsnitt 7) |
...
| **Kategorier av personuppgifter** | Namn, adress, organisationsnummer (för enskild firma = personnummer), bankgiro/kontonummer, belopp och betalningsinformation, fakturauppgifter, fritext i bankhändelser, underlagens fullständiga innehåll (kvitton, fakturor, avtal), avsändaradress för inkommande underlag samt (sedan 2026-09-09) resultatet av avsändarkontrollen för inmejlade underlag: avsändar- och kuvertdomän, SPF/DKIM/DMARC-utfall, metod och orsak — aldrig rubriktext (`documents.avsandare_verifierad`, `documents.avsandare_kontroll`; bevaras med underlaget). |
...
| **Säkerhetsåtgärder (art. 32)** | RLS per bolag; oföränderlig bokföring (BFL-skydd v3–v6: bokförda verifikationer kan inte ändras eller raderas, rättelse sker genom omvänd verifikation, rättelsejournal append-only); periodlås; audit-triggrar; privata buckets med storleks- och MIME-begränsning; slumpmässigt sjusiffrigt arkivnummer som mottagaradress; avsändarkontroll av inmejlade underlag (2026-09-09: alignment mellan autentiserad domän enligt Cloudflares SPF/DKIM/DMARC-resultat och synlig avsändare — obekräftad avsändare ger granskningsstatus utan automatisk AI-tolkning, underlaget tas alltid emot); promptskydd som behandlar dokumentets text som data, inte instruktioner (OCR-prompt v8); nattlig avstämning databas–Storage; dagliga databasbackuper (Storage ingår inte) samt extern säkerhetskopia av Storage-objekten till Azure Blob Storage i Sweden Central i oföränderlig container (`backup-underlag`, i drift sedan 2026-09-09 med nattlig körning 02:30 och driftvakt — se B8 och `docs/BACKUP-UNDERLAG.md` i kundappen); frivillig tvåfaktorsautentisering per användare (TOTP via Supabase Auth, 2026-09-09) med kodsteg i alla BokPilots appar, obligatorisk i databasen för plattformsadministratörer så snart en faktor finns, och återställning efter identitetskontroll som loggas i konsolens append-only-logg med orsak och kontrollmetod och meddelas användaren per e-post (`docs/MFA.md` i kundappen). Se avsnitt 7. |
...
| **Delflöden** | **B4a — tolkning av underlag** (`tolka-underlag`, `inbound-email`, Claude Haiku 4.5): underlagets bild/PDF, kontoplan och eventuellt OCR-textlager skickas — aldrig KYC-data, historik eller kunddata utöver själva underlaget; inmejlade underlag vars avsändare inte kunnat bekräftas (avsändarkontrollen, B1) skickas inte automatiskt utan först efter manuell granskning. **B4b — assistent och chattar** (`assistent-ai`, `bokforingsassistent`, `bokfor-ai`, `granska-ai`, `manadskontroll-ai`, `bokslut-ai`, `annual-report-ai`, `ekonomichef-ai`, `robo-bp-chat`; Haiku 4.5/Sonnet 5): bankhändelsernas fritext (innehåller regelmässigt motpartsnamn och uppgift om löneutbetalningar), verifikationer och huvudbok, kund- och leverantörsfakturor med motpartsuppgifter, underlagstolkningar samt användarens frågor — inte personnummer ur lönemodulen, inte KYC-material. **B4c — Spiris-bryggan** (klienter i Visma eAccounting/Fortnox): pseudonymiserade leverantörsfakturarader utan fritext; egen förteckning 2026-08-03. **B4d — MCP-connector** (`mcp-server`): läs- och skrivverktyg via användarens egen Claude-klient med tvåstegsbekräftelse; aktiverad per bolag (`company_ai_features.claude_connector`), i dag endast för testbolag. |
...
| **Gallringstid** | `assistent_logg`, `robo_bp_messages`, `support_ai_events`: 24 månader (fastställd 2026-08-17, automatisk). `ai_bokforing_logg`: bevaras med bokföringen (BFL). `ai_call_log`: 12 månader (beslut 2026-09-09, automatisk gallring, se A6). `ai_usage_log`, `mcp_audit_log`, `bokslut_ai_suggestions`, utkast till årsredovisning: [KLAMMER — ingår inte i gallringsjobbet]. Hos Anthropic: enligt ovan. |
| **Säkerhetsåtgärder (art. 32)** | Dataminimering per flöde (dokumenterad i `_shared/claudeChat.ts` och `_shared/claudeOcr.ts`); KYC-tabeller läsbara enbart för byråmedlemmar; strukturerade svar med fast schema; tidsgränser och omförsök; kvoter och nedkylning — tolkning högst 8 anrop per användare och minut (`ai_claim_job`, `ai_cooldowns`), AI-assistenterna högst 60 anrop per användare och timme, 400 per dygn och 1 500 per timme för hela plattformen (`ai_kvot_klaim`, `ai_call_log`; åtgärdat 2026-09-09 efter edge-granskningens fynd 2026-08-25); människa-i-loopen före bokföring; funktionsgrindar per bolag för robo-bp, AI-bokslut och connector — **tolkning och assistent kan inte stängas av per bolag**, vilket ska framgå av biträdesavtalet; MCP: tvåstegsbekräftelse, användarens egen RLS, append-only-logg. Se avsnitt 7. |
...
| **System och lagringsplats** | Supabase Storage (buckets `underlag`, `arkiv`, `annual-report-exports`) och databas — Stockholm; Supabase dagliga backuper (Pro-plan, 7 dagars retention; **Storage ingår inte i backuperna**); extern säkerhetskopia av Storage-objekten i Azure Blob Storage, Sweden Central (lagringskonto `bokpilotarkiv`, ZRS, Cold, container-level WORM 3 000 dagar per räkenskapsår, container `underlag-2026`; verifieringsdokument 2026-08-26) — **i drift sedan 2026-09-09** (nattlig kopiering 02:30, provkörning 29 av 29 objekt; kvarhållningspolicyn låses efter första nattkörningen); oberoende andrakopia [KLAMMER]; byråns klientmappar i Microsoft 365 [KLAMMER]. |
| **Mottagare / underbiträden** | Supabase, Inc.; Microsoft Ireland Operations Ltd (Azure — i drift sedan 2026-09-09; M365). |
...
**D6. Säkerhetskopiering, kontinuitet och lagringsintegritet.** Dagliga databasbackuper hos Supabase (Pro-plan, 7 dagars retention) [KLAMMER — ta ställning till point-in-time recovery]; Storage ingår inte i backuperna, vilket kompenseras av nattlig avstämning databas–Storage med larm vid saknade filer och, sedan 2026-09-09, av den nattliga oföränderliga säkerhetskopian av Storage-objekten i Azure Sweden Central (driftvakt larmar vid utebliven eller misslyckad kopiering); oberoende andrakopia utanför Azure [KLAMMER]; säkrad betalning och eskaleringsrutin för arkivabonnemanget; återläsningstest [KLAMMER — intervall].
