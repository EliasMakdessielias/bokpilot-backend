alter table public.user_companies add column if not exists moduler text[];
...
revoke all on function public.byra_skapa_klient(uuid, text, text, text, text) from public, anon;
grant execute on function public.byra_skapa_klient(uuid, text, text, text, text) to authenticated;
