-- Följdändring av bfl_skydd_lon_och_levfaktura_v1.
--
-- Den nya triggern skyddar motparten på en bokförd leverantörsfaktura (BFL 5 kap.
-- 7 § — verifikationen ska ange motparten). reset_company nollade supplier_id
-- respektive customer_id när registren rensades, vilket därmed skulle blockeras
-- mitt i körningen med ett svårbegripligt fel.
--
-- Rätt beslut är inte att försvaga skyddet utan att göra återställningen
-- konsekvent: har bolaget bokföring får inte heller kund- och
-- leverantörsregistret rensas, eftersom bokförda fakturor pekar på posterna.
-- I ett bolag utan bokföring fungerar allt som förut.
