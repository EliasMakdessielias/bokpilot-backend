...
| Sökväg | Innehåll |
|---|---|
| `supabase/migrations/` | Alla 70 migrationer ur `supabase_migrations.schema_migrations`, en fil per migration (`<version>_<namn>.sql`) |
...
| | *Schemafilerna uppdaterade 2026-08-25 efter etapp 4–7 (search_path-låsning + anon-indragning, FK-index, RLS-InitPlan, behörighetskoll utan uid-beroende, anon utan tabellrättigheter; 249 funktioner, 75 triggrar)* |
| `schema/constraints.sql` | PK/FK/unique/check-constraints |
| `schema/indexes.sql` | De 191 index som inte backar constraints |
| `schema/functions.sql` | Alla 249 egna databasfunktioner (exkl. extension-ägda) |
| `schema/triggers.sql` | Alla 75 triggrar |
...
| `schema/cron_jobs.sql` | De 6 pg_cron-jobben |
...
- Cron-jobben innehåller projektets **anon-nyckel** i klartext — den är publik per design, men mappen ligger i `Projekt\` (utanför OneDrive-synk) i linje med hur kodprojekt hanteras på den här datorn.
