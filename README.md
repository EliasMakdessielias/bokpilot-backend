...
Ögonblicksbild av backendens hela kodbara innehåll, hämtad **2026-08-20** via Supabase-MCP från projektet `bokpilot-sverige` (ref `vzeqvapebkbapwflozbi`, eu-north-1, org `wtxdxhnzcxrbzpvysdyh`). Senast synkad mot databasen **2026-09-09** (etapp 11b/11c: driftvakten bedömer bara avslutade körningar, `imap-import` avregistrerad; kundappens migrationer `byrastod_scope_v1`, `ai_kvot_v1`, `inbound_avsandare_v1`, `ai_kvot_v2`, `backup_underlag_v1`, `backup_underlag_v2`, `gdpr_gallring_ai_call_log_v1`, `notis_inkommande_kalla_v1`, `mfa_aal2_plattformsadmin_v1` och `beta_ansokan_orgnr_v1`; edge-funktionerna efter GitHub Actions-driftsättningen 2026-09-09 kl. 09:46 UTC, då `backup-underlag` tillkom som nummer 33).

## Innehåll

| Sökväg | Innehåll |
|---|---|
| `supabase/migrations/` | Alla 99 migrationer ur `supabase_migrations.schema_migrations`, en fil per migration (`<version>_<namn>.sql`), MD5-verifierade mot databasen |
...
| `schema/functions.sql` | Alla 265 egna databasfunktioner (exkl. extension-ägda) |
